...
\usepackage{textcomp}
\usepackage[table]{xcolor} % Unificado aqui com a opção [table] para consertar o cellcolor
\usepackage{scalerel}
\usepackage{eqparbox}
...
\usepackage{calc}
% \usepackage{array} removido pois estava duplicado
...
\usepackage{float}
\usepackage{etoolbox}
% \usepackage{nicematrix} removido pois causa erro de versão e não está sendo utilizado
\usepackage{bibentry}
...
\begin{document}
	%	\fphead
	%	
	%	% Cabeçalho corrigido com \parbox para evitar erros de compilação da coluna
	...
	%	\begin{tabular}{|c|c|c|}
		...
		%		\hline
		%		\multicolumn{3}{l}{$^{(1)}$ medido a 25$^{\circ}$C} \\
		%	\end{tabular}
	%\end{table}
