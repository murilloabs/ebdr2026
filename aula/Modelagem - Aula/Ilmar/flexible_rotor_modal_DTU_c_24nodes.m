...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%      AJUSTING PLOT SCALES & ANGLE VIEW       %
%             view(angleX,angleY)              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   max_ampl_plot=0.15;
   angleX=20
...
% Bearing 2 - Stiffness
 ...
 % all shaft elements building to total shaft length

 for i=1:NE+1,
     shaftnodeposition(i)=0;
 end
    
  for i=2:NE+1,
     shaftnodeposition(i)=shaftnodeposition(i-1)+l(i-1);
  end
...
% Adding the stiffness matrices of the bearing elements
 ...
 %[U,lambda]=eig(inv(Mglob)*Kglob);
 [lam,p]=sort(diag(lambda));
 U=U(:,p);

 % Printing the Nfirst=20 Eigenvectors and Eigenvalues
 Nfirst=20;
 lam(1:Nfirst)/2/pi
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 for i=1:(NE+1)
  ...
  hold on
  for cont=1:NE+1,
 %  plot3(pos1(cont,:),w(cont,:),v(cont,:),'k','LineWidth',2.5);
   plot3(pos1(cont,:),w(cont,:),v(cont,:),'k','LineWidth',2.5);
  end
  ...
  lengthoftheshaft=sum(l);

  axis([0 lengthoftheshaft -max_ampl_plot max_ampl_plot -max_ampl_plot max_ampl_plot])
  ModoVirt=input([' Enter the number of the mode shape to be plotted, zero to esc, highest mode ',maximo,': ']);
  ModoReal=2*ModoVirt;
  figure(ModoVirt)
